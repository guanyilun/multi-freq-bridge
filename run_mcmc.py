...
def get_initial_params(cf, n_walkers):
    ...
    vdelta_init = np.random.uniform(cf['v_delta_min'], cf['v_delta_max'], size=n_walkers)
    ...
    if cf["model_choice"] == "fit_vavg":
        ...
        ret_array = np.array([
            ...
            # Average velocity
            vavg_init,          # 25 - Average velocity
            vdelta_init         # 26 - Velocity delta
...
def lnprior(theta):  
    ...
    if cf["model_choice"] == "fit_vavg":  
        ...
        check_vavg = (cf['v_avg_min'] < theta[25] < cf['v_avg_max'])
        check_vdelta = (cf['v_delta_min'] < theta[26] < cf['v_delta_max'])

        
        if check_c1 and check_c2 and check_fil and check_vavg and check_vdelta:
...
def lnlike(theta):
    ...
    for idx in range(len(cf['data'])):
        ...
        fit_model = np.fft.fft2((c1_model + c2_model + fil_model) * apod_mask) * beam_list_array[idx] / mean_apod_mask)
