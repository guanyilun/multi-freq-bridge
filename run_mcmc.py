   ...
   # c1_v_init = np.random.uniform(cf['c1_v_min'], cf['c1_v_max'], size=n_walkers)
    ...
    if cf["model_choice"] == "fit_vavg":
        ...
        ret_array = np.array([
            ...
            # Cluster 2: Abell 399
            c2_ra_init,         # 7 - Right Ascension
            c2_dec_init,        # 8 - Declination
            c2_beta_init,       # 9 - Beta
            c2_rc_arcmin_init,  # 10 - Core radius (arcminutes)
            c2_R_init,          # 11 - Radius
            c2_theta_init,      # 12 - Theta
            c2_Dtau_init,       # 13 - Optical depth

            # Filament
            fil_ra_init,        # 14 - Right Ascension
            fil_dec_init,       # 15 - Declination
            fil_l0_init,        # 16 - Length (pixels)
            fil_w0_init,        # 17 - Width (pixels)
            fil_Dtau_init,      # 18 - Optical depth

            vavg_init           # 19 - Velocity
...
def lnprior(theta):  
    ...
    if cf["model_choice"] == "fit_vavg":  
        ...
        check_c2 = (c2_ra_min_pix < theta[7] < c2_ra_max_pix
                    and c2_dec_min_pix < theta[8] < c2_dec_max_pix
                    and cf['c2_beta_min'] < theta[9] < cf['c2_beta_max']
                    and cf['c2_rc_arcmin_min'] < theta[10] < cf['c2_rc_arcmin_max']
                    and cf['c2_R_min'] < theta[11] < cf['c2_R_max']
                    and cf['c2_theta_min'] < theta[12] < cf['c2_theta_max']
                    and cf['c2_Dtau_min'] < theta[13] < cf['c2_Dtau_max'])
                    #and cf['c2_Te_min'] < theta[16] < cf['c2_Te_max']
                    #and cf['c2_A_D_min'] < theta[17] < cf['c2_A_D_max'])
        
        check_fil = (fil_ra_min_pix < theta[14] < fil_ra_max_pix
                    and fil_dec_min_pix < theta[15] < fil_dec_max_pix
                    and fil_l0_min_pix < theta[16] < fil_l0_max_pix
                    and fil_w0_min_pix < theta[17] < fil_w0_max_pix
                    and cf['fil_Dtau_min'] < theta[18] < cf['fil_Dtau_max']
                    #and cf['fil_Te_min'] < theta[23] < cf['fil_Te_max']
                    #and cf['fil_A_D_min'] < theta[24] < cf['fil_A_D_max']
                    )
        
        check_v_avg = cf['v_avg_min'] < theta[19] < cf['v_avg_max']
